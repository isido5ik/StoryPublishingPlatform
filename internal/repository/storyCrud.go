...
func (r *repository) CreateStory(story dtos.AddPostInput, userId int) (int, error) {
	...
	query := fmt.Sprintf(`
		INSERT INTO %s (user_id, title, content, category_id) 
		VALUES ($1, $2, $3, $4) 
		RETURNING post_id`, postsTable)

	row := r.db.QueryRow(query, userId, story.Title, story.Content, story.CategoryID)
...
func (r *repository) GetStories(pagination dtos.PaginationParams) ([]dtos.Post, error) {
	...
	query := fmt.Sprintf(`
		...
		FROM 
			%s p 
		JOIN 
			%s u ON p.user_id = u.user_id 
		JOIN 
			%s c ON c.category_id = p.category_id 
		LIMIT %d OFFSET %d`,
		postsTable, usersTable, categoryTable, pagination.PageSize, offset)
...
func (r *repository) GetStoriesByCategory(pagination dtos.PaginationParams, categoryId int) ([]dtos.Post, error) {
	var stories []dtos.Post
	...
	if err := r.db.Select(&stories, query, categoryId); err != nil {
		logrus.WithError(err).Error("Failed to select all stories")
		return nil, err
	}
